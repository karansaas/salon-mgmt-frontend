...
export const createEmployeeLoginAccount = async ({ id, email, password }: { id: string; email: string; password: string }): Promise<EmployeeLoginAccount> => (await api.post<{ account: EmployeeLoginAccount }>(`/employees/${id}/account`, { email, password })).data.account;
export const resetEmployeeLoginPassword = async ({ id, password }: { id: string; password: string }): Promise<EmployeeLoginAccount> => (await api.put<{ account: EmployeeLoginAccount }>(`/employees/${id}/account/password`, { password })).data.account;
