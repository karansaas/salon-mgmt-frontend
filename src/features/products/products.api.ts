import { api } from '../../lib/api'; import type { Product, ProductHistory, ProductInput, ProductListResponse } from './types';
export const getProducts = async (params: { page: number; search: string; category: string; brand: string; status: string }): Promise<ProductListResponse> => (await api.get<ProductListResponse>('/products', { params: { ...params, limit: 10, sortBy: 'createdAt', sortOrder: 'desc', category: params.category || undefined, brand: params.brand || undefined } })).data;
...
export const getLowStockProducts = async (): Promise<Product[]> => (await api.get<{ products: Product[] }>('/products/low-stock')).data.products;
